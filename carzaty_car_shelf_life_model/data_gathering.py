...
padding = max(len(final_encoded_dictionary[value][0]) for value in final_encoded_dictionary)
padding_dictionary = OrderedDict()

for value in final_encoded_dictionary:
    padding_value = padding - int(final_encoded_dictionary[value][0].shape[0])
    padding_dictionary[value] = np.pad(np.array(final_encoded_dictionary[value]),
                                       [(0, 0), (0, padding_value)], mode="constant")

numpy_array_to_be_entered_in_model = np.array(padding_dictionary["make_en"])

for value in padding_dictionary:
    if value != "make_en":
        numpy_array_to_be_entered_in_model = np.dstack((numpy_array_to_be_entered_in_model,
                                                        padding_dictionary[value]))
x_train, x_test, y_train, y_test = train_test_split(numpy_array_to_be_entered_in_model,
...
optimizer = Adagrad(lr=0.01, decay=0.000001)

model.compile(metrics=["mae"], loss="logcosh",
              optimizer=optimizer)

model.fit(x_train, y_train, epochs=5000, validation_split=0.03)
